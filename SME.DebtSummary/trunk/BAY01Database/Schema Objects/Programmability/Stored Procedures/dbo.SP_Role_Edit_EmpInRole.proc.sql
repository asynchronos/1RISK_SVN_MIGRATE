...
-- =============================================
CREATE PROCEDURE [dbo].[SP_Role_Edit_EmpInRole]
...
BEGIN
	...
	BEGIN TRY
		BEGIN TRANSACTION
			...
			DELETE FROM RoleGroup_EmpInRole
			WHERE RoleGroup_EmpInRole.EMP_ID=@EMP_ID
				AND EXISTS (SELECT *
							FROM RoleGroup_Role AS R
							WHERE R.ROLE_CATEGORY_ID=@CategoryId
								AND R.ROLE_ID=RoleGroup_EmpInRole.ROLE_ID)
				AND NOT EXISTS (SELECT *
								FROM #TMP
								WHERE #TMP.ROLE_ID=RoleGroup_EmpInRole.ROLE_ID)

			-- Insert Roles
			INSERT INTO RoleGroup_EmpInRole
				   ([EMP_ID]
				   ,[ROLE_ID])
			(
			 ...
			 WHERE NOT EXISTS (SELECT *
								...
								WHERE E_IN_R.EMP_ID=@EMP_ID
									AND E_IN_R.ROLE_ID=#TMP.ROLE_ID)
			 )
