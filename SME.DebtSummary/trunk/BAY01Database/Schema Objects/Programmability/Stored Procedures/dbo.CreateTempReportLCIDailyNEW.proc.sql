...
-- =============================================
CREATE PROCEDURE [dbo].[CreateTempReportLCIDailyNEW]
...
BEGIN
	...
	SET NOCOUNT ON;
  Drop table Temp_Report_LciDaily_New;
    ...
    right outer join 
	 ...
	 (	SELECT   lc.cif,cus.cus_title + cus.cus_first + ' ' + cus.cus_last 'custname',max_aging 'aging',
		class_h,cus.busi_type,busi_desc,aging_h
		FROM    DBF_IMPORT.dbo.TEMP_MAX_AGING_EDW as LC 
	  	 left outer join BAY01.dbo.customer as cus on LC.CIF=CUS.CIF
		 left outer join DBF_IMPORT.dbo.rep_prod  as REP ON  LC.cif=REP.cif
		WHERE   LC.cif NOT IN 
		(SELECT CIF_NO FROM  DBF_IMPORT.DBO.TMaster_2_Last_Register)
        AND max_aging > 0.01
      ) as LCI
    ON LCI.cif=SME.CIF 
