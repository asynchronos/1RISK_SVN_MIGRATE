...
-- =============================================
CREATE  PROCEDURE [dbo].[Report_Credit_By_Customer]
    @Year  nvarchar(4),
    @Month nvarchar(2),
	@Day   nvarchar(2),
	@YearR  nvarchar(4),
...
set @strTableName ='TEMP_REPORT_PRODUCT_CUS'
...
if @Group ='0' 
	...
	Begin
		SET @CRI2 =N'  AND (PROCESS_GROUP=''B'' OR PROCESS_GROUP=''A'') '  
	End	
else if @Group ='C' 
	Begin
...
if @Level_id='0'
   ...
   --   SET @CRI5 = N' AND ([EMP-ID] IN ( SELECT DISTINCT EMP_ID  FROM  control_line ))'
   -- แก้ให้ดึงข้อมูลจากสายงานใหม่
      SET @SQL=N''
...
if  @NotFin ='False'
   ...
   SET @SQL = @SQL + 'SELECT * FROM ' + @strTableName  + ' WHERE 0=0 '  + @CRI1 + @CRI2 + @CRI3 + @CRI4 + @CRI5 + @CRI6 + @CRI7 + ' order by id,[APP-DATE]'
