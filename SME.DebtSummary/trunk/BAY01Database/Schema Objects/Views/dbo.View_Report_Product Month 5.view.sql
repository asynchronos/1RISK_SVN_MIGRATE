CREATE VIEW dbo.[View_Report_Product Month 5]
AS
WITH cte AS (SELECT     ID, CUS_TITLE, CUS_FIRST, CUS_LAST, EMP_ID, EMPNAME, EMPSURNAME, PROCESS_ID, PROCESS, DECB_RD, VALUE_APPROVE, 
                                                    ...
                                                    Id_Branch, Branch_T, Id_Region, Region_Name, TITLE_NAME, Send_To_Board, LEVEL_NAME
                             FROM         dbo.v_Credit_Account_Process
                             WHERE     (MONTH(PROCESS_DATE) = 5))
    SELECT     TOP (100) PERCENT ID,
                                ...
                                (SELECT     TOP (1) ISNULL(TITLE_NAME, '') + ISNULL(EMPNAME, '') + ' ' + ISNULL(EMPSURNAME, '') AS Expr1
                                  ...
                                  WHERE      (ID = A.ID) AND (PROCESS_DATE =
                                                             (SELECT     MAX(PROCESS_DATE) AS Expr1
                                                               FROM          cte AS ANNALS_CREDIT_PROCESS_1
                            ...
                            CASE WHEN
                                ...
                                (SELECT     ISNULL(SUM(CASE WHEN Process_Group = 'D' THEN VALUE_APPROVE ELSE Value_Customer END), 0) AS Expr1
                                  ...
                                  WHERE      (ID = A.ID) AND (DECB_RD_ID NOT IN ('0001', '0002', '0003', '0004', '0007')) AND (SUBJECT_DETAIL_ID IN ('101', '102', '103')) AND 
                                                         (PROCESS_DATE =
                                                             (SELECT     MAX(PROCESS_DATE) AS Expr1
                                                               FROM          cte AS ANNALS_CREDIT_PROCESS_3
                                ...
                                (SELECT     ISNULL(SUM(CASE WHEN Process_Group = 'D' THEN VALUE_APPROVE ELSE Value_Customer END), 0) AS Expr1
                                  ...
                                  WHERE      (ID = A.ID) AND (DECB_RD_ID NOT IN ('0001', '0002', '0003', '0004', '0007')) AND (SUBJECT_DETAIL_ID IN ('104', '105')) AND 
                                                         (PROCESS_DATE =
                                                             (SELECT     MAX(PROCESS_DATE) AS Expr1
                                                               FROM          cte AS ANNALS_CREDIT_PROCESS_2
                                                               WHERE      (ID = A.ID)))) AS [(OTHERS)],
                                (SELECT     ISNULL(SUM(CASE WHEN Process_Group = 'D' THEN VALUE_APPROVE ELSE Value_Customer END), 0) AS Expr1
                                  FROM          cte AS v
                                ...
                                (SELECT     MAX(PROCESS) AS Expr1
                                  ...
                                  WHERE      (ID = A.ID) AND (PROCESS_DATE =
                                                             (SELECT     MAX(PROCESS_DATE) AS Expr1
                                                               FROM          cte AS ANNALS_CREDIT_PROCESS_2
                                                               WHERE      (ID = A.ID)))) AS PROCESS, CASE WHEN
                                (SELECT     COUNT(PROCESS_DATE)
                                  FROM          cte AS v
                                  WHERE      (ID = A.ID AND Process_id = 'B070')) >= 1 THEN
                                (SELECT     datediff(day,
                                                             ...
                                                             (SELECT     MAX(PROCESS_DATE)
                                                               FROM          cte AS v
                                                               WHERE      (ID = A.ID AND Process_id = 'B070')))) + 1 ELSE
                                (SELECT     datediff(day,
                                                             (SELECT     MIN(PROCESS_DATE)
                                                               FROM          cte AS v
                                                               WHERE      (ID = A.ID)),
                                                             (SELECT     MAX(PROCESS_DATE)
                                                               FROM          cte AS v
                                                               WHERE      (ID = A.ID)))) + 1 END AS TTY, CASE WHEN Send_To_Board = 'True' OR
...
Begin DesignProperties = 
   Begin PaneConfigurations = 
      Begin PaneConfiguration = 0
         NumPanes = 4
         Configuration = "(H (1[16] 4[22] 2[39] 3) )"
      End
   ...
   Begin CriteriaPane = 
      ...
      End
   End
End', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'View_Report_Product Month 5';


GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'View_Report_Product Month 5';
