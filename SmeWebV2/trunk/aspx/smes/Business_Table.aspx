...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        function returnValue(BUSINESS_ID, BUSINESS_NAME
            ...
            window.parent.$("#" + BUSINESS_GROUP_TABLETextBox)[0].value = BUSINESS_GROUP;
...
<body>
    ...
    <asp:SqlDataSource runat="server" ID="BusinessTypeSqldatasource" ConnectionString="<%$ ConnectionStrings:BAY01ConnectionString %>"
        SelectCommand="SELECT * FROM [SME_S].[BUSINESS_TYPE] "></asp:SqlDataSource>
            ...
            <asp:TemplateField ShowHeader="False">
                <ItemTemplate>
                    <asp:ImageButton ID="imgLocation" runat="server" ImageUrl="../../images/qmv_bb_preview.gif"
                        Height="22px" Width="22px" ToolTip="เลือกลูกค้า" 
                        OnClientClick='<%# "returnValue("""+Eval("BUSINESS_ID").toString() + """,""" + Eval("BUSINESS_NAME").toString() +""","""+Eval("BUSINESS_GROUP").toString()+ """,""" + Eval("MARGIN").toString() + """,""" + Eval("DSCR").toString() + ""","""  + Eval("AP").toString() + """,""" + Eval("AR").toString() + """,""" + Eval("STOCK").toString() + """,""" + Eval("LTV").toString() + """,""" + Eval("WORKING_CAPITAL").toString() + """,""" + Eval("RATE").toString() + """,""" + Eval("CONTRACT").toString() +""") ; return false;" %>' />
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
