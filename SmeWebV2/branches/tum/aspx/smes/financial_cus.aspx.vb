...
Partial Class smes_financial_cus
    ...
    Sub showCustomerGrid()
        ...
        numCustomer.Text = (CusDT.Rows.Count)

        If CusDT.Rows.Count < 1 Then
            Dim CusDR As DataRow = CusDT.NewRow()
            CusDR("CIF") = System.DBNull.Value
            CusDR("Name") = ""
            CusDR("BusinessCode") = ""
            CusDR("BusinessName") = ""
            CusDT.Rows.Add(CusDR)
    ...
    Sub bindCustomerGrid()
        CusDT = New DataTable
        CusDT.Columns.Add("CIF", Type.GetType("System.Int32"))
    ...
    Sub ShowData(ByVal smes_id As Integer)
        ...
        If reader.HasRows Then
            ...
            Dim CifTable As New DataTable
            CifTable.Load(reader)
            CusDT = CifTable
            ' Session("CusDT") = CifTable
            showCustomerGrid()
        End If
    ...
    Sub UpdateData()
        ...
        For i As Integer = 0 To CusDT.Rows.Count - 1
            With CusDT.Rows(i)
                ...
                sqlCmd.Parameters.AddWithValue("@SMES_ID", SMES_IDTextBox.Text)
                sqlCmd.Parameters.AddWithValue("@CIF", .Item("CIF"))
                sqlCmd.Parameters.AddWithValue("@BUS_CODE", .Item("BusinessCode"))
